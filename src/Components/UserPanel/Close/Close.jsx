...
const Close = () => {
  ...
  useEffect(() => {
    const fetchETAs = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5174/tickets/eta-by-mobile/${mobileNumber}`);
        const etaResults = {};
        ...
        response.data.tickets.forEach(eta => {
          ...
          // Updated logic to show hours if less than or equal to 24, otherwise show days
          etaResults[eta.ticketId] = totalHours > 24 ? `${totalDays} days` : `${totalHours} hours`;
        });
  ...
  const handleCheckboxChange = (ticketNo) => {
    ...
    updatedSelectedTickets.has(ticketNo) ? updatedSelectedTickets.delete(ticketNo) : updatedSelectedTickets.add(ticketNo);
    ...
    <div className="flex flex-col mt-20 ml-32 h-full w-[88%]">
      <div className="flex justify-between items-center bg-white h-20">
        <div className="flex items-center mb-4">
          <span className="mt-2" style={{ fontFamily: "Roboto", fontSize: "18px", fontWeight: "400", lineHeight: "28px", color: "#343A40" }}>
            All ({totalEntries})
          </span>
        ...
        <div className="flex flex-row gap-3 mr-3">
          <img src="/search.png" alt="Search Icon" className="h-7 w-7" onClick={handleSearch} />
          <img src="/setting.png" alt="Setting Icon" className="h-7 w-7" />
          <img src="/filter.png" alt="Filter Icon" className="h-7 w-7" />
        </div>
      </div>
                    ...
                    <tr>
                      <th scope="col" className="px-2 py-2">
                        <input
                          type="checkbox"
                          onChange={(e) => {
                            setSelectedTickets(e.target.checked ? new Set(currentTickets.map(ticket => ticket.ticketNo)) : new Set());
                          }}
                        />
                      </th>
                      {["Ticket No.", "Date", "Time", "Category", "Issue Description", "ETA", "Preview", "Chat", "Download", "Track"].map(header => (
                        <th key={header} className="px-4 py-2 font-roboto text-[#343A40] text-[14px] font-[700] leading-[22px] text-left">{header}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {currentTickets.map((ticket) => {
                      ...
                      const etaBackgroundColor = getEtaBackgroundColor(eta.includes('days') ? parseInt(eta) : 0);
                      return (
                        <tr key={ticket.ticketNo} className="border-b border-neutral-200 bg-white transition duration-300 ease-in-out hover:bg-neutral-100">
                          ...
                          <td className="whitespace-nowrap px-2 py-2 font-medium text-neutral-900">
                            <div className="flex justify-center">
                              <img src="/track.png" alt="Track" className="h-7 w-7" />
                            </div>
                          </td>
                        </tr>
                      );
